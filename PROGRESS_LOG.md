...
## [pre] — v0.3.7-pre1 — API Source selector ported from BannerHub v2.7.4 (2026-03-27)
**Tag:** v0.3.7-pre1 (pending)
**What changed:**
- New `BhApiSelectorHelper.java` — `getApiSource()`, `setApiSource()`, `getEffectiveApiUrl()`, `showApiSelectorDialog()`
- Settings → Advanced: new "API Source" button (contentType=0x65) below "Grant Root Access"
- Tapping "API Source" shows a 2-option radio dialog: BannerHub API (default) / EmuReady Proxy
- `EggGameHttpConfig.c(Context)` intercepted at runtime to read effective URL from pref instead of build-time constant
- 4 new patches in build.yml (22–25): SettingItemViewModel, SettingItemEntity, SettingBtnHolder, EggGameHttpConfig
**Files touched:**
- `extension/BhApiSelectorHelper.java` (new)
- `.github/workflows/build.yml` (patches 22–25 added)

---

## [stable] — v0.3.6 — Install confirm, cancel download, exe picker, Set .exe (2026-03-26)
**Tag:** v0.3.6  |  **Commit:** `24c2b1e`
**What changed:**
- Install confirmation dialog (size + free storage before download)
- Cancel download button (red during download, stops + deletes partial files)
- Exe picker post-install (auto if 1; dialog if 2+; full exclusion list)
- Set .exe button in game detail dialog
- fetchGameSize fix: sums depot sizes from manifest (not nonexistent build fields)
- README updated
**Files touched:** `extension/GogDownloadManager.java`, `extension/GogGamesActivity.java`, `README.md`
**CI result:** ✅ run 23602770862

---

## [fix] — v0.3.6-pre — Fix fetchGameSize showing unknown (2026-03-26)
**Tag:** v0.3.6-pre (retagged)  |  **Commit:** `24c2b1e`
**What changed:**
- fetchGameSize() was looking for size_before_download/compressed_size on build items (don't exist)
- Fix: fetch manifest URL from build item, decompress, sum depot[].size fields (matches BannerHub)
- Gen1 fallback: reads total_size from build item
**Files touched:** `extension/GogDownloadManager.java`
**CI result:** ✅ run 23601887775

---

## [pre] — v0.3.6-pre — Port BannerHub v2.7.3 GOG features (2026-03-26)
**Tag:** v0.3.6-pre  |  **Commit:** `4c48e4f`
**What changed:**
- GogDownloadManager: startDownload() returns cancel Runnable; AtomicBoolean cancelled + installDirRef
- Callback.onSelectExe() default method added
- Gen2+Gen1+Installer: cancelled checks, installDirRef.set, onSelectExe at end
- collectExeCandidates() replaces findExe() — full exclusion list
- fetchGameSize() + formatBytes() added
- downloadWithProgress() checks cancelled flag
- showInstallConfirm() — size + free storage dialog before every download
- showExePicker() — picker dialog for ambiguous exes
- All 3 install surfaces: showInstallConfirm wrap, Cancel button (red), cancel Runnable stored
- showDetailDialog(): custom LinearLayout + .exe label + Set .exe… button
**Files touched:** `extension/GogDownloadManager.java`, `extension/GogGamesActivity.java`
**CI result:** ✅ run 23601578681 (1m38s)
